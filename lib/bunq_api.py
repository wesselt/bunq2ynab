...
def add_callback(bunq_user_id, url_end, url):
    ...
    set_callbacks(
        bunq_user_id, url_end, {"category": "MUTATION", "notification_target": url}
    )
...
def callback_str(cb):
    return "{}:{}".format(cb["category"], cb["notification_target"])


def set_callbacks(bunq_user_id, url_end, new_callback):
    old_callbacks = [
        nf_to_callback(nf) for nf in get_notification_filters(bunq_user_id)
    ]
    new_callbacks = []
    found = False
    dirty = False
...
def get_user_id(user_name):
    for u in bunq.get("v1/user"):
        for k, v in u.items():
            if (
                v["display_name"].casefold() == user_name.casefold()
...
def get_account_id(user_id, account_name):
    ...
    for entry in reply:
        ...
        if account["status"] == "ACTIVE" and (
            account["description"].casefold() == account_name.casefold()
...
def put_callbacks(user_id, new_notifications):
    data = {"notification_filters": new_notifications}
    method = "v1/user/{}/notification-filter-url".format(user_id)
